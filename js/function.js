...
$(function () {
   ...
   $loading.children('p').fadeOut(4000);
...
$btnnext.on('click', function (evt) {
   ...
   $container.stop().animate({ left:'-100%' }, 400,'easeInOutCubic',function(){
      ...
      $slides.first().appendTo($container);//마지막 li로이동 //무한슬라이더
        $container.css({left:0});
   });
});

//이전버튼에 대한 click 이벤트구문
$btnprev.on('click', function (evt) {
   ...
   const $slides = $('#uxdesign > .slides > .slides-container > li'); //li 4개
   $slides.last().prependTo($container); //첫번째 li로 이동//무한슬라이더
